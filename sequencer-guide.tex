...
Let $d = \{b_i, v_i\}_{1}^{n}$ be a sequence of $n$ beat signatures, where $v_i$ is the BPS  value at beat $b_i$. 
Let $ f: \mathbb{R} \rightarrow \mathbb{R} $ be a function that provided a beat, returns the seconds from the first beat. We define this function as a $n$-step piecewise function
    \begin{equation}
	    f(x) = \begin{dcases}
		    f(b_{i-1}) + \frac{x-b_{i-1}}{v_i}\,, & \text{if $b_{i-1} < x \leq b_{i}\,;\quad \forall i=1,\ldots,n$}\,; \\
	    \end{dcases}
	    \label{eq:beat2second}
    \end{equation}
    where $b_0 := 0$ and $f(b_0) := 0$.
    ...
    \begin{equation}
	    g(x) = \begin{dcases}
		    \left[x-f(b_{i-1})\right]\times v_i + b_{i-1}\,, & \text{if $f(b_{i-1}) < x \leq f(b_{i})\,;\quad \forall i=1,\ldots,n$}\,, \\
	    \end{dcases}
	    \label{eq:beat2second}
    \end{equation}
    where $g = f^{-1}$.
