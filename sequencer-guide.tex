...
Let $ \mathcal{T} =  \left\{\left( b_i^{(t)}, r_i \right)\right\}_{i=1}^{n} $ be a sequence of STOPS or DELAYS, where $ r_i $ is the stop or delay (measured in seconds) at beat $ b_i^{(t)} $. Let $ f^{-1}:\mathbb{B}\rightarrow \mathbb{S^{*}} $ be a function that retrieves the second from the start of the song given a current beat, where $ \mathbb{B} = \mathbb{R} $, and $ \mathbb{S^{*}} = \mathbb{R}$.
...
Now, this is great! By asking $p$, now we have the effective beat, and therefore the position. Note that de drawing position IS NOT the beat for that we are going to use to calculate when the note is needed to be tapped!
    ...
    \begin{equation}
	    p(x) = \int g(x) dx = \begin{dcases}
		    ...
		    p(b^{(s)}_i) + (x-b^{(s)}_i)\times s_i\,, & \text{if $ b^{(s)}_{i} < x \leq b^{(s)}_{i+1}\,; \quad \forall i=2,\dots,n'$}\,,
	    \end{dcases}
    ...
    However there are three gimmicks that modify this scroll function in a number of different ways: \texttt{\#WARPS}, \texttt{\#SPEEDS}, and \texttt{\#SCROLLS}. Let us see and example to know what these modifiers do. Suppose we have the following \texttt{\#WARPS} definition:
    \begin{verbatim}
    #WARPS:4=2;     
    \end{verbatim}
    ...
    \begin{verbatim}
  {
    ...
    We have already talked about how the \texttt{\#SCROLLS} definition changes where the notes should be placed on the screen. It turns out that also has an impact on the scrolling function. Taking the same example from Section \ref{sec:beat2noteposition}, we can extract the following information:
    ...
    We would like to have a function $ q: \mathbb{R}\rightarrow \mathbb{R} $ that calculates the warped beat value for a given beat as well as its inverse, $ q^{-1} $, a function $ e: \mathbb{R}\rightarrow \mathbb{R} $ that calculates the speed factor at a given beat, and a function $ g: \mathbb{R}\rightarrow \mathbb{R} $ that calculates the scroll value.

    \subsection{Solution}

    \paragraph{Warps}
    First, let us deal with the problem of the WARPS. We would need to create a function that maps from beats to warped beats first, so we could retrieve the warped beat value w.r.t. it.
...
\begin{axis}[
  axis x line=middle, axis y line=middle,
  ymin=0, ymax=15, ytick={0,...,15}, ylabel=Warped Beat,
  xmin=0, xmax=15, xtick={0,...,15}, xlabel=Beat,
]
\addplot[blue, domain=0:15, samples=400]{func(x)};
\end{axis}
...
\begin{axis}[
  axis x line=middle, axis y line=middle,
  ymin=0, ymax=15, ytick={0,...,15}, ylabel=Beat,
  xmin=0, xmax=15, xtick={0,...,15}, xlabel=Warped Beat,
]
\addplot[blue, domain=0:15, samples=400]{func(x)};
\end{axis}
\end{tikzpicture} 
...
To ease the understanding, Table \ref{tab:symbol-table} gathers all the functions that we are going to need as well as a brief descripcion for each one of them.
    ...
    \begin{table}[htbp]
	    ...
	    \begin{tabular}{lp{11cm}}
		    ...
		    $ \mathbb{W} $ & Warped Beat space (units) $ \mathbb{W} = \mathbb{R} $.\\
	    ...
	    $ t_{(s)}^{-1}$ & Function  $ t_{(s)}^{-1}: \mathbb{S^{*}}\rightarrow \mathbb{D} $ that maps from the the sequencer time space into the delayed time space.\\
		    ...
		    $ q $ & Function $ q: \mathbb{B}\rightarrow \mathbb{W} $ that maps from the beat space into the warped beat space.\\
		    $ q^{-1} $ & Function $ q^{-1}: \mathbb{W}\rightarrow \mathbb{B} $ that maps from the warped beat space into the beat space.\\
    ...
    \begin{equation}
	    ...
	    \label{eq:set}
    \end{equation}
    where $ w: \mathbb{W}\rightarrow \mathbb{S} $
    \begin{equation}
	    w(x) = \left( t_{(d)}^{-1}\circ t_{(s)}^{-1}\circ f^{-1}\circ q^{-1} \right)(x)\,,
	    \label{eq:final-g}
    \end{equation}
    ...
    \begin{equation}
	    \left[ w_i - \left(g\circ q\circ f\circ t_{(s)}\circ t_{(d)} \right)(t) \right] \times \left( e\circ q\circ f\circ t_{(s)}\circ t_{(d)} \right)(t)\,.
